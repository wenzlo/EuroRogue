...
public interface IColoredString<T> extends Iterable<IColoredString.Bucket<T>> {
     ...
     */
    ArrayList<IColoredString.Bucket<T>> getFragments();
    ...
    class Bucket<T> {
        ...
        public IColoredString.Bucket<T> append(String text) {
            if (text == null || text.isEmpty())
                ...
                return new IColoredString.Bucket<>(this.text + text, color);
        }
        ...
        public IColoredString.Bucket<T> setLength(int l) {
            final int here = text.length();
            if (here <= l)
                return this;
            else
                return new IColoredString.Bucket<>(text.substring(0, l), color);
        }

        public IColoredString.Bucket<T> setColor(T t) {
            return color == t ? this : new IColoredString.Bucket<T>(text, t);
        }
